...
app.get('/api/portfolio/summary', async (_req, res) => {
    try {
        const symbols = db.prepare('SELECT symbol FROM stocks').all().map((r) => r.symbol);
        if (symbols.length === 0)
            return res.json({ totalValue: 0, positions: [] });
        ...
        const positions = (pricesResp.data.symbols || []).map((row) => ({ symbol: row.symbol?.replace('.US', '') ?? '', price: Number(row.close) || 0 }));
        const totalValue = positions.reduce((sum, p) => sum + p.price, 0);
